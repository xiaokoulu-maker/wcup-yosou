        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{marginBottom:10,display:"inline-block",position:"relative",
            ...
            <img src={LOGO_IMG} alt="W杯予想メーカー"
              style={{width:86,height:86,borderRadius:14,objectFit:"contain",background:"#061533",
...
function RoomPredictTab({t,nav,update,myId,navToRanking}){
  ...
  const isJoined=!!me;
  const deadlinePassed=isDeadlinePassed(t.deadline);
  const BASE_PRED={winner:"",runnerUp:"",topScorer:"",assistKing:"",tournamentMvp:"",japanMvp:"",japanResult:"",favoriteCountry:"",comment:""};
  const [pred,setPred]=useState(me?.predictions?{...BASE_PRED,...me.predictions}:{...BASE_PRED});
  const [err,setErr]=useState("");
  ...
  const lineShareText=encodeURIComponent(`私の2026W杯優勝予想は【${pred.winner||"？"}】！\n日本代表は【${pred.japanResult||"？"}】予想です。\nあなたも予想してみて👇\n${url}\n#W杯予想`);
  const copyUrl=async()=>{try{await navigator.clipboard.writeText(url);}catch{}setCopied(true);setTimeout(()=>setCopied(false),2000);};
  const save=async()=>{
    const s2=getSettings(t);
    const missing=(s2.winner&&!pred.winner)||(s2.japanResult&&!pred.japanResult)||!pred.favoriteCountry;
    if(missing){setErr("必須項目を選択してください");return;}
    setLoading(true);
    ...
    trackEvent("save_tournament_prediction",{tournamentId:t.id});
  ...
  if(!isJoined)return(
    <div style={{padding:"16px 18px 0"}}>
      <div className="card lg" style={{textAlign:"center",padding:"28px 16px"}}>
        <div style={{fontSize:36,marginBottom:12}}>⚽</div>
        <div style={{color:"var(--gold)",fontWeight:900,fontSize:16,marginBottom:8}}>大会に参加してから予想できます</div>
        <div style={{color:"var(--muted)",fontSize:13,lineHeight:1.7,marginBottom:16}}>大会に参加すると予想を入力できます。</div>
  ...
  if(saved)return(
    <div style={{padding:"16px 18px 0"}}>
      <div className="card lg" style={{borderColor:"rgba(55,214,122,.35)",background:"rgba(55,214,122,.04)"}}>
        <div style={{textAlign:"center",marginBottom:16}}>
          <div style={{fontSize:28,marginBottom:4}}>🎉</div>
          <div style={{color:"#37d67a",fontWeight:900,fontSize:17}}>予想完了！</div>
          <div style={{color:"var(--muted)",fontSize:12.5,marginTop:6}}>
            🥇 優勝: <strong style={{color:"var(--txt)"}}>{pred.winner||"—"}</strong>
            {pred.japanResult&&<> ／ 🇯🇵 日本: <strong style={{color:"var(--txt)"}}>{pred.japanResult}</strong></>}
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:9}}>
          <a href={`https://line.me/R/msg/text/?${lineShareText}`} target="_blank" rel="noopener noreferrer"
             className="btn btn-line lg" style={{textDecoration:"none"}}
             onClick={()=>trackEvent("share_pred_line",{tournamentId:t.id})}>
      ...
      <button className="btn btn-dark sm" style={{width:"100%",marginTop:10,color:"var(--muted)"}} onClick={()=>setSaved(false)}>
        ✎ 予想を編集する
      </button>
    </div>
  );
  ...
  return(
    <div style={{padding:"16px 18px 0"}}>
      ...
      <div className="card" style={{marginTop:0}}>
        <div style={{color:"var(--txt)",fontWeight:800,fontSize:14,marginBottom:8}}>📅 試合スコア予想</div>
        <div style={{color:"var(--muted)",fontSize:12,marginBottom:10}}>全104試合の勝敗スコアを予想してポイントを稼ごう。</div>
        <button className="btn btn-dark md" style={{width:"100%"}} onClick={()=>nav("matches")}>
          ⚽ もっと詳しく予想する →
        </button>
      </div>
    ...
    <div className="screen">
      ...
      <div className="wrap section tight">
        <div className="card lg" style={{textAlign:"center"}}>
          <img src={LOGO_IMG} alt="ロゴ" style={{width:52,height:52,borderRadius:10,objectFit:"contain",background:"#061533",marginBottom:8,boxShadow:"0 0 0 3px rgba(62,123,255,.2),0 4px 16px rgba(30,70,200,.15)"}}/>
...
/* ── 全国ランキング（全体scope用） ── */
function PgGlobalRanking({nav,navDashboard,myId}){
  ...
  const myEntry=myId?list.find(r=>r.id===myId):null;
    ...
    <div className="screen">
      ...
      <div className="wrap section tight">
        <div className="banner blue" style={{alignItems:"flex-start",gap:8}}>
          ...
          <div style={{fontSize:12}}>大会エントリ単位での集計です。同一ユーザーが複数エントリで並ぶ場合があります。</div>
        </div>
      </div>
        ...
        <div className="wrap section tight">
          <div className="card lg" style={{textAlign:"center"}}>
            ...
            {myPts>0?(
              <>
                {myRank
                  ?<><div style={{fontFamily:"'Roboto Mono',monospace",fontWeight:900,fontSize:48,color:"var(--gold)",lineHeight:1}}>#{myRank}</div>
                    <div style={{color:"var(--muted)",fontSize:13,marginTop:4}}>/ {total.toLocaleString()} 人中</div></>
      ...
      {err&&!loading&&(
        <div className="wrap section tight">
          <div className="banner blue" style={{flexDirection:"column",gap:8}}>
            <span>{err}</span>
            <button onClick={()=>load(true)} className="btn btn-dark sm" style={{width:"auto",padding:"6px 14px"}}>再試行</button>
          </div>
        </div>
      )}
  ...
  if(b11page==="top")return(
    <div style={{paddingBottom:40}}>
      <div style={{background:"linear-gradient(180deg,#061533 0%,#0a1f4c 100%)",padding:"50px 22px 40px",textAlign:"center",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        <img src={LOGO_IMG} alt="ロゴ" style={{width:72,height:72,borderRadius:12,objectFit:"contain",background:"#061533",marginBottom:10,boxShadow:"0 0 0 3px rgba(0,104,183,0.2),0 4px 20px rgba(0,91,172,0.15)"}}/>
    ...
    <div style={{paddingBottom:40}}>
      ...
      <div ref={previewRef} style={{margin:"0 auto",width:"min(340px,90vw)",background:"linear-gradient(180deg,#061A0D 0%,#0A2E14 40%,#061A0D 100%)",borderRadius:22,overflow:"hidden",boxShadow:"0 12px 48px rgba(0,0,0,0.7),0 0 0 1px rgba(247,201,72,0.15)",border:`1.5px solid rgba(247,201,72,0.25)`}}>
        ...
        <div style={{padding:"12px 20px",textAlign:"center",borderTop:`1px solid ${G.gold}22`,marginTop:12}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><img src={LOGO_IMG} alt="ロゴ" style={{width:22,height:22,borderRadius:4,objectFit:"contain",background:"#061533"}}/><span style={{color:G.gold,fontSize:11,fontWeight:700}}>W杯予想メーカー</span></div>
