// ds-ui.jsx — redesign shared React components
// Converted from design_handoff_wcup_redesign/ui.jsx to ES module
...
export const TEAMS = {
  ...
  "イタリア": ["ITA", "#0066af"], "メキシコ": ["MEX", "#006847"], "カナダ": ["CAN", "#ff0000"],
  "モロッコ": ["MAR", "#c1272d"], "エクアドル": ["ECU", "#fcd116"], "コスタリカ": ["CRC", "#002b7f"],
  "セネガル": ["SEN", "#00853f"], "オーストラリア": ["AUS", "#00843d"], "スイス": ["SUI", "#ff0000"],
  "デンマーク": ["DEN", "#c60c30"], "サウジアラビア": ["KSA", "#006c35"], "日本": ["JPN", "#d7224b"],
};

export function Flag({ name, lg }) {
...
export function BackRow({ onClick, label = "戻る" }) {
  return (
    <div className="backrow" onClick={onClick}>
      <Icon name="back" size={17} stroke={2.2} /> {label}
    </div>
  );
}

export function PageHead({ onBack, eyebrow, title, icon, right }) {
  return (
    <div className="phead">
      <div className="b" onClick={onBack}><Icon name="back" size={18} stroke={2.2} /></div>
      <div className="t">
...
export function SectionHead({ title, gold, action, onAction }) {
  return (
...
export function Tabs({ items, value, onChange, gold }) {
  return (
...
export function Banner({ tone = "blue", icon, children }) {
  return (
...
export function MatchCard({ group, home, away, ko, cd, dt, odds, reward, onPredict, predicted }) {
  return (
    <div className="match">
      <div className="top">
